...
export const Arrow = React.forwardRef<HTMLDivElement, {}>((props, ref) => {
  return (
    <div
      ref={ref}
      className="absolute z-20 h-4 w-4 rotate-45 mt-0.5 ml-[1.2rem] bg-white calendar-popover-tail"
    />
  );
});
...
export const RoundedButton: React.FC<Button> = ({
  ...
  const getClassName = useCallback(() => {
    const defaultClass = !roundedFull
      ? `w-full tracking-wide transition-all duration-300 px-3 ${padding} uppercase hover:bg-gray-100 rounded-md focus:ring-1`
      : `transition-all duration-300 hover:bg-gray-100 rounded-2xl p-[0.45rem] focus:ring-1`;
